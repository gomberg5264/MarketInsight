...
const batchSummary = asyncHandler(async (req, res) => {
  ok(isString(req.query.symbols), 'symbols param is not a string');
  
  let cached;

  try {
    cached = await cache.get(req.query.symbols);
  } catch (err) {
    cached = false;
  }
  ...
  const symbols = split(',', req.query.symbols);
  ...
  try {
    ...
    if (/^API limit reached$/.test(err.message)) {
...
const querySymbol = asyncHandler(async (req, res) => {
  ok(isString(req.query.query), 'query param is not a string');
  
  let cached;
  
  try {
    cached = await cache.get(req.query.query);
  } catch (err) {
    cached = false;
  }
  ...
  try {
    const matchPromise = matchClosestSymbol(req.query.query);
    ...
    } else if (/^API limit reached$/.test(err.message)) {
      return res.status(403).json({
        error: err.message
      });
    }
  ...
  await cache.set(req.query.query, match);
