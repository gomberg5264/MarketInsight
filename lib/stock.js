...
if (!API_TOKEN) { // fail fast if no API_KEY is provided
  throw new Error('No API_TOKEN value defined. Obtain one at https://iexcloud.io');
}
...
const { 
  ...
  IEX_MAX_PERCENTAGE_USAGE,
  MAX_BATCH_QUERYABLE_SYMBOLS,
} = require('./constants');
...
async function matchClosestSymbol (query, max=10) {
  ...
  const batchAdditionalPromise = authenticateAndFetch(
    `${IEX_BASE_API}/stock/market/batch?symbols=${encodeURIComponent(join(',', lowercaseArray(reduceToProp('symbol', subset))))}&types=quote,logo`
  );
  ...
  return map((stock) => ({
    ...
    quote: {
      latestPrice: batchAdditional[stock.symbol].quote.latestPrice
    }
...
async function getBatchSummary (symbols, since='2y') { // max is 2y data
  ...
  // Check length is greater than max queryable
  if (length(uppercaseSymbols) > MAX_BATCH_QUERYABLE_SYMBOLS) {
  ...
  const stockBatchPromise = authenticateAndFetch(
    `${IEX_BASE_API}/stock/market/batch?symbols=${encodeURIComponent(join(',', lowercaseArray(normalizedSymbols)))}&types=${join(',', STOCK_BATCH_INTERESTED_ENDPOINTS)}&range=${since}`
  );
...
async function authenticateAndFetch (url, token = API_TOKEN) {
  if (process.env.NODE_ENV === 'production') {
    ...
    // Prevents account from calling beyond set limit
    const limitReached = accountQuota.messagesUsed >= (accountQuota.messageLimit * (IEX_MAX_PERCENTAGE_USAGE / 100)) 
      || accountQuota.messagesUsed >= accountQuota.messageLimit;
