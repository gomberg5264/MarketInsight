...
// Enable TLS if certs are provided for mutual auth
if (process.env.REDIS_TLS_CERT && process.env.REDIS_TLS_KEY && process.env.REDIS_TLS_CACERT) {
  redisConfig = Object.assign({}, redisConfig, {
    tls: {
      cert: readFileSync(process.env.REDIS_TLS_CERT),
      key: readFileSync(process.env.REDIS_TLS_KEY),
      ca: readFileSync(process.env.REDIS_TLS_CACERT)
    }
  })
}
exports.DEFAULT_REDIS_CONFIGURATION = Object.freeze(redisConfig);
// Paths
