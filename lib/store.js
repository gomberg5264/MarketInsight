...
const { createRedisClient } = require('./db');
...
class BasicMarketWatchStore extends MarketWatch {
  ...
  static async start (options) {
    ...
    const store = new BasicMarketWatchStore(options);
...
class RedisMarketWatchStore extends MarketWatch {
  ...
  async stop (force = true) {
    const stopPromise = this._redisClient.quit();
    await stopPromise;
    super.stop(force);
  }
  ...
  static async start (options) {
    ok(isObject(options), 'start: options is not an object');
    // Initialize new instance
    const store = new RedisMarketWatchStore(options);
