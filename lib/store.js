...
class MarketWatch extends Server {
  ...
  static async init (store) {
    ...
    limiter.on('error', (err) => debug(err));
    limiter.on('failed', () => debug('job failed'));
    limiter.on('dropped', (dropped) => debug(`dropped req: ${dropped}`));
    // Setup store
    ...
    for await (const session of sessionIterator) {
      ...
      session.on('sync', (message) => limiter.schedule(async () => {
        try {
...
class BasicMarketWatchStore extends MarketWatch {
  ...
  static async start (options) {
    ...
    await MarketWatch.init(new BasicMarketWatchStore(options));
  }
}
...
class RedisMarketWatchStore extends MarketWatch {
  ...
  static async start (options) {
    ok(isObject(options), 'start: options is not an object');
    // Initialize new instance
    await MarketWatch.init(new RedisMarketWatchStore(options));
  }
}
