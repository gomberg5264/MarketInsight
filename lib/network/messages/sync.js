...
class SyncMessage extends Message { 
  ...
  validate () {
    return Message.validateSchema(this.payload, SyncMessage.schema) //&& isStringArray(payload) 
      && (this.payload.length || this.payload.every((value) => typeof value === 'string' && value.length && !/^\s+$/.test(value)));
  }
