...
class ErrorMessage extends Message { 
  ...
  static validate (payload) {
    return Message.validateSchema(payload, ErrorMessage.schema) && !isEmpty(payload.message);
  }

  pack () {
    if (!(this.payload instanceof Error)) {
      throw new TypeError('pack: payload is not an error');
    }
    ...
    if (!ErrorMessage.validate(this.payload)) {
