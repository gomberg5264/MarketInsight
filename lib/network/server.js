...
const { generateIdentifier } = require('../util');
...
class Server extends EventEmitter {
  ...
  // TODO: allow array of allowed domains
  _validateClient (headers = {}) {
    return headers.hasOwnProperty('origin') 
      && ((process.env.NODE_ENV !== 'production' 
        && headers['origin'].indexOf('localhost') > -1)
      || headers['origin'].indexOf('herokuapp.com') -1);
  }
  
  async _handleIncomingConnection (socket) {
    const session = ServerSession.bind(socket);
    const headers = session.socket.upgradeReq.headers;
    // Validate headers are coming from a trusted source
    if (!this._validateClient(headers)) {
      debug('Invalid client found: Destroying socket');
  ...
  broadcast (message) {
    this._sessions.forEach((session) => session.write(message));
  }
