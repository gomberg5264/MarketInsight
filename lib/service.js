...
class RedisMarketWatchService extends MarketWatchService {
  constructor (options) {
    ...
    this._symbolsKey = `${REDIS_KEY}_symbols`;
  }
  ...
  async _subscribe (symbol) {
    ...
    // Add member to set
    const addMemberResultPromise = this._store.addToSet(this._symbolsKey, normalizedSymbol);
  ...
  async _unsubscribe (symbol) {
    ...
    const removeMemberPromise = this._store.removeFromSet(this._symbolsKey, normalizedSymbol);
  ...
  async update (symbols) {
    ...
    const symbolsResultPromise = this._store.iterateSet(this._symbolsKey, 0, MAX_WATCH_LIMIT);
  ...
  async setup (symbols = DEFAULT_STORE_SYMBOLS) {
    ...
    const removeAllPromise = this._store.drop(this._symbolsKey);
  ...
  static async start (options) {
    ...
    const initializePromise = MarketWatchService.init(
      new Bottleneck({
        ...
        id: `${APP_NAME}_${APP_VERSION}`,
        datastore: 'ioredis',
        clientOptions: DEFAULT_REDIS_CONFIGURATION,
        clearDatastore: true
      }), 
      store
    );
